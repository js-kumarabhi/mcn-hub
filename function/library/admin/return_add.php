				...
				if(!empty($isbn)){
					...
					if($query->num_rows > 0){
						...
						if($query->num_rows > 0){
							...
							$borrow_id = $borrow['id'];
							$sql = "INSERT INTO returns (student_id, book_id, date_return,collegeCode) VALUES ('$student_id', '$bid', NOW(),'$code')";
							if($conn->query($sql)){
								$return++;
								$sql = "UPDATE books SET status = 0 WHERE bookid = '$bid'";
								$conn->query($sql);
